...
const required = [
  ...
  'public/assets/v86/ui/bottom_nav_clean.png','public/assets/v86/ui/recent_panel_clean.png','public/assets/v86/ui/reel_panel_clean.png','public/assets/v86/ui/tab_blue.png','public/assets/v86/ui/tab_gold.png'
];
...
if (!data.includes("APP_VERSION = '8.6.0'")) fail('APP_VERSION is not 8.6.0');
if (!data.includes('aqua-fantasia-v8.6.0-kakao-portrait-ui-cleanup')) fail('CACHE_NAME mismatch');
const pkg = JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8'));
if (pkg.version !== '8.6.0') fail('package version mismatch');
...
if (guard.includes('screen.orientation') || guard.includes('orientation.lock')) fail('screen orientation lock must not be used');
const manifest = JSON.parse(fs.readFileSync(path.join(root,'public/manifest.webmanifest'),'utf8'));
if (manifest.display === 'fullscreen') fail('manifest display must not be fullscreen');
if (manifest.orientation !== 'portrait-primary') fail('manifest must keep portrait-primary');
if (!ok) process.exit(1);
console.log('[validate-clean] Aqua Fantasia v8.6.0 clean single README package OK');
console.log(JSON.stringify({ ok:true, version:'8.6.0', files: walk(root).length }, null, 2));
