...
let ok = true;
const fail = (msg) => { console.error('[check-v650]', msg); ok = false; };
...
for (const token of ['6.5.1', 'mangrove', 'lunar', 'reefFestival', 'tide', 'unlockHint', 'mastery', 'lastRescueAt']) if (!data.includes(token)) fail(`data token missing ${token}`);
...
for (const token of ['fallback-scene', 'surge-meter', 'guard-active', 'weather-pill', 'system-strip']) if (!css.includes(token)) fail(`css token missing ${token}`);
for (const token of ['bg_mangrove.webp', 'aqua_fishing_atlas_v650.webp', 'aqua-fantasia-v6.5.1-portrait-lock-hotfix']) if (!sw.includes(token)) fail(`service worker token missing ${token}`);
...
if (pkg.version !== '6.5.1') fail('package version must be 6.5.1');
const atlas = JSON.parse(read('public/assets/atlas/aqua_fishing_atlas_v650.json'));
if (Object.keys(atlas.frames ?? {}).length < 12) fail('v6.5 atlas metadata too small');
if (!ok) process.exit(1);
console.log('[check-v650] portrait lock + massive resilience expansion OK');
