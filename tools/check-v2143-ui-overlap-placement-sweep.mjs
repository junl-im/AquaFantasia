...
const fail = (message) => { console.error(`[v2143] ${message}`); process.exit(1); };
...
if (pkg.version !== '2.1.43') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.43' || lock.packages?.['']?.version !== '2.1.43') fail('package-lock.json version mismatch');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.43'", 'APP_VERSION 2.1.43');
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.43-ui-overlap-placement-sweep', 'v2.1.43 cache name');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.43-ui-overlap-placement-sweep', 'service worker cache');
assertIncludes('public/offline.html', 'v2.1.43', 'offline badge');
assertIncludes('README.md', '# AquaFantasia v2.1.43', 'README title v2.1.43');
...
for (const token of [
  ...
  'v2143-ui-overlap-placement-sweep-root',
  'v2143-runtime-page-shell',
  'v2143-aqua-page',
  'v2143-bottom-nav',
  'v2143-fishing-focus-screen',
  'v2143-fishing-stage',
  'v2143-fishing-hud',
  'v2143-sea-lane-card',
  'v2143-fishing-loadout',
  'v2143-fishing-priority-hint',
  'v2143-bite-action',
  'data-v2143-bite-action',
  'v2143-reel-panel',
  'v2143-reel-console',
  'v2143-reel-touch-zone',
  'v2143-cast-button',
  'fishing-safe-stack-page-card-hud-placement-audit',
  'transparent-frame-audit-zero-overlap-fixed-size',
]) {
  if (!main.includes(token)) fail(`src/main.ts missing v2.1.43 token: ${token}`);
}
...
for (const token of [
  'V2143_TILE_TOUCH_OVERLAP_AUDIT_LOCK',
  'V2143_DIAMOND_TOUCH_SCORE_LIMIT = 0.96',
  'bestScore <= V2143_DIAMOND_TOUCH_SCORE_LIMIT',
  'Full tile shrink remains deferred until save/building/NPC/camera migration is implemented',
  'dataset.v2143TileTouchOverlapAuditLock',
  'const maxAssistRadius = Math.min(V2138_FINE_PLACEMENT_SEARCH_RADIUS, 2)',
  'const TILE_W = 80',
  'const TILE_H = 40',
]) {
  if (!village.includes(token)) fail(`src/villageWorld.ts missing v2.1.43 tile/placement token: ${token}`);
}
...
for (const token of [
  ...
  '.v2143-bottom-nav',
  '.v2143-runtime-page-shell',
  '.v2143-runtime-page-shell .runtime-content',
  '.v2143-fishing-hud',
  '.v2143-sea-lane-card',
  '.v2143-fishing-loadout',
  '.v2143-fishing-priority-hint',
  ...
  '.v2143-reel-console',
  '.v2143-reel-touch-zone',
  '.v2143-fishing-focus-screen[data-fishing-phase="reeling"] .v2143-reel-panel:not(.hidden)',
  '--v2143-fishing-reserved',
  'body[data-screen="fishing"] .bottom-nav',
]) {
  if (!styles.includes(token)) fail(`src/styles.css missing v2.1.43 style token: ${token}`);
}

for (const token of [
  'UI 겹침, 마을 배치, 낚시 콕핏 가독성',
  '로드/미끼 스트립, 바다물길 카드, 우선 안내, 캐스팅 CTA, 릴 게이지, 릴 콘솔, 터치존',
  ...
  if (!readme.includes(token)) fail(`README.md missing v2.1.43 note: ${token}`);
}
...
console.log('[v2143] UI overlap, placement, fishing, HUD, menu guard passed');
