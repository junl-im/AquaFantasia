...
if (!(index.includes('v57-water-art-direction.js?v=5.7.0') || index.includes('v57-water-art-direction.js?v=5.8.0') || index.includes('v57-water-art-direction.js?v=5.9.0')) || !index.includes('v5.7.0 Water Art Direction Pass')) fail('index v5.7 runtime marker missing');
if (!(index.includes("const APP_VERSION = '5.7.0'") || index.includes("const APP_VERSION = '5.8.0'") || index.includes("const APP_VERSION = '5.9.0'"))) fail('index APP_VERSION must be 5.7.0 or later');
...
if (!(sw.includes('aqua-fantasia-v5.8.0-2-5d-art-20260612') || sw.includes('aqua-fantasia-v5.9.0-state-dex-20260612') || sw.includes('aqua-fantasia-v5.7.0-water-art-20260612')) || !sw.includes('./src/runtime/v57-water-art-direction.js')) fail('service worker v5.7 cache entry missing');
...
if (!(manifest.includes('v5.8.0 2.5D Art Reboot') || manifest.includes('v5.7.0 Water Art Direction')) || !(manifest.includes('5.8.0-2-5d-art-20260612') || manifest.includes('5.9.0-state-dex-20260612') || manifest.includes('5.7.0-water-art-20260612'))) fail('manifest v5.7 markers missing');
if (!pkg.includes('runtime57:check') || !pkg.includes('check-v57-water-art.mjs')) fail('package v5.7 script missing');
if (!(state.includes("APP_VERSION = '5.7.0'") || state.includes("APP_VERSION = '5.8.0'") || state.includes("APP_VERSION = '5.9.0'")) || !state.includes("background: 'assets/art/v57_fishing_bg_lake_master.webp'") || !((state.includes('aqua_v5.8.0') || state.includes('aqua_v5.9.0')) || state.includes('aqua_v5.7.0'))) fail('core state v5.7 version/background markers missing');
