...
const fail = (message) => { console.error(`[v2137] ${message}`); process.exit(1); };
...
if (pkg.version !== '2.1.37') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.37' || lock.packages?.['']?.version !== '2.1.37') fail('package-lock.json version mismatch');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.37'", 'APP_VERSION 2.1.37');
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.37', 'v2.1.37 cache name');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.37', 'service worker cache');
assertIncludes('public/offline.html', 'v2.1.37', 'offline badge');
assertIncludes('README.md', '# AquaFantasia v2.1.37', 'README title v2.1.37');
...
for (const token of [
  ...
  if (!main.includes(token)) fail(`src/main.ts missing v2.1.37 token: ${token}`);
}
for (const token of [
  'V2137_TILE_TOUCH_PRECISION_LOCK',
  'V2137_TILE_PIXEL_SIZE_MIGRATION_REQUIRED_LOCK',
  'V2137_FINE_PLACEMENT_SEARCH_RADIUS = 2',
  'bestScore <= 1.14',
  'Full tile pixel shrink is intentionally locked',
  'nearestPlaceableOrigin',
  'hasVisualObjectClearance',
]) {
  if (!village.includes(token)) fail(`src/villageWorld.ts missing v2.1.37 token: ${token}`);
}
for (const token of [
  'v2137-fishing-ui-page-shell-hud-menu-polish-root',
  '--v2137-card-bg',
  ...
  if (!styles.includes(token)) fail(`src/styles.css missing v2.1.37 token: ${token}`);
}
for (const token of [
  '낚시 화면 우선 개편',
  '로드/미끼 장비 테이블',
  ...
  if (!read('README.md').includes(token)) fail(`README.md missing v2.1.37 note: ${token}`);
}
...
console.log('[v2137] premium fishing UI/page shell guard passed');
