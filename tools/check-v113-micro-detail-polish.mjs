...
add(pkg.version === '1.1.3', 'package version is 1.1.3');
add(data.includes("APP_VERSION = '1.1.3'"), 'APP_VERSION is 1.1.3');
add(data.includes('aqua-fantasia-v1.1.3-micro-detail-polish'), 'cache name updated in data');
add(sw.includes('aqua-fantasia-v1.1.3-micro-detail-polish'), 'service worker cache updated');
...
if (failed.length) {
  console.error(`v1.1.3 micro detail polish validation failed: ${failed.length} issue(s)`);
  process.exit(1);
}
console.log('v1.1.3 micro detail polish checks passed.');
