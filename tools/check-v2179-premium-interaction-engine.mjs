...
const fail = (msg) => { console.error(`[v2179] ${msg}`); process.exit(1); };
...
if (pkg.version !== '2.1.79') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.79' || lock.packages?.['']?.version !== '2.1.79') fail('package-lock.json version mismatch');
if (!pkg.scripts?.validate?.includes('check-v2179-premium-interaction-engine.mjs')) fail('validate script must run v2.1.79 guard');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.79'");
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.79-premium-interaction-engine');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.79-premium-interaction-engine');
assertIncludes('public/offline.html', 'v2.1.79');
assertIncludes('README.md', '# AquaFantasia v2.1.79');
...
for (const token of [
  ...
  'installV2179PremiumInteractionEnginePass',
  'dataset.v2179PremiumInteractionEngine',
  'v2179-premium-interaction-engine-root',
  'v2179-opening-video-only',
  'v2179-opening-video',
  'only-video-no-bubble-no-frame-no-poster',
  'posterless-autoplay-fullscreen',
  "video.removeAttribute('poster')",
  'v2179-fishing-interaction-screen',
  'gauge-mid-upper-reel-bottom-loadout-left-stack',
  'v2179-battle-strip',
  'v2179-reel-console',
  'v2179-single-reel-button',
  'v2179-sea-lane-card',
  'v2179-fishing-loadout',
  'v2179-bite-callout',
  'v2179-action-badge',
  'v2179-premium-aqua-card',
  'v2179-shop-card',
  'v2179-text-budget',
  'v2179-scroll-safe-content',
  'v2179-img-policy',
  'document.hidden',
...
for (const token of [
  '/* v2.1.79: premium interaction engine',
  '.v2179-opening-video-only',
  '.v2179-opening-video',
  'body[data-screen="village"] .village-world-screen[data-v2179-opening-contract="video-only-before-village-chrome"]',
  'body[data-screen="fishing"] .v2179-fishing-interaction-screen',
  '.v2179-sea-lane-card',
  '.v2179-fishing-loadout',
  '.v2179-battle-strip',
  'top: var(--v2179-gauge-top) !important;',
  '.v2179-reel-console:not(.hidden)',
  'min-height: var(--v2179-reel-console-height) !important;',
  '.v2179-single-reel-button',
  '.v2179-bite-callout',
  '.v2179-premium-aqua-card',
  '.v2179-shop-card',
  '.v2179-scroll-safe-content',
  'img[data-v2179-img-policy]',
...
console.log('[v2179] premium interaction engine, video-only intro contract, player entry camera recenter guard, fishing no-overlap lanes, card/menu budget, performance guards, and direction/package checks passed');
