...
if (pkg.version !== '8.9.0') fail('package version mismatch');
if (!data.includes("APP_VERSION = '8.9.0'")) fail('APP_VERSION is not 8.9.0');
if (!data.includes('aqua-fantasia-v8.9.0-v3d-underwater-runtime-ui')) fail('CACHE_NAME mismatch');
console.log('[validate-clean] Aqua Fantasia v8.9.0 clean single README package OK');
console.log(JSON.stringify({ ok:true, version:'8.9.0', files: files.length }, null, 2));
