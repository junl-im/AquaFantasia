...
if (!data.includes('fishing-dialog-close-unification') && !data.includes('grounded-motion-polish') && !data.includes('loop-ui-button-audit')) fail('CACHE_NAME must keep v2.0.59+ cache lineage');
if (!sw.includes('fishing-dialog-close-unification') && !sw.includes('grounded-motion-polish') && !sw.includes('loop-ui-button-audit')) fail('service worker cache must keep v2.0.59+ cache lineage');
