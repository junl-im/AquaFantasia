...
const fail = (msg) => { console.error(`[v2172] ${msg}`); process.exit(1); };
...
if (pkg.version !== '2.1.72') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.72' || lock.packages?.['']?.version !== '2.1.72') fail('package-lock.json version mismatch');
if (!pkg.scripts?.validate?.includes('check-v2172-system-content-engine.mjs')) fail('validate script must run v2.1.72 guard');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.72'");
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.72-system-content-engine');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.72-system-content-engine');
assertIncludes('public/offline.html', 'v2.1.72');
assertIncludes('README.md', '# AquaFantasia v2.1.72');
assertIncludes('README.md', '## v2.1.72 변경사항');
...
console.log('[v2172] system/content/engine upgrade, fishing lane separation, cards/dock/shop/build guards, performance containment, direction assets, and package guards passed');
