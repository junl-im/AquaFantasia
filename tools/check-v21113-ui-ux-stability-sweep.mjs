...
const fail = (message) => { console.error(`[v21113] ${message}`); process.exit(1); };
const must = (condition, message) => { if (!condition) fail(message); };
const version = '2.1.113';
const cache = 'aqua-fantasia-v2.1.113-ui-ux-stability-sweep';
...
must(pkg.scripts.validate === 'node tools/clean-old-patch-docs.mjs && node tools/validate-clean.mjs && node tools/check-v21113-ui-ux-stability-sweep.mjs', 'validate script must use the v2.1.113 UI/UX stability sweep guard');
must(data.includes(`APP_VERSION = '${version}'`) && data.includes(cache), 'data.ts version/cache mismatch');
must(sw.includes('v2.1.113') && sw.includes(cache), 'service worker not synced');
must(offline.includes('v2.1.113') && offline.includes('모바일 세로 UI/UX 안정성'), 'offline page not synced');
must(readme.startsWith('# AquaFantasia v2.1.113') && readme.includes('## v2.1.113 변경사항'), 'README version/history missing');
must(handoff.includes('기준 패키지 버전: `2.1.113`') && handoff.includes('v2.1.113 UI/UX 안정성 스윕'), 'handoff v2.1.113 record missing');

must(main.includes("classList.add('v21113-ui-ux-stability-root')"), 'v2.1.113 root class missing');
must(main.includes("dataset.v21113UiUxStability = 'safe-area-card-touch-scroll-design-sweep'"), 'v2.1.113 root dataset missing');
must(css.includes('v2.1.113 UI/UX stability sweep'), 'v2.1.113 CSS section missing');
must(css.includes('--v21113-card-width') && css.includes('--v21113-wide-card-width'), 'v2.1.113 card width variables missing');
must(css.includes('focus-visible') && css.includes('touch-action: manipulation'), 'touch/focus UX guard missing');
must(css.includes('overscroll-behavior: contain') && css.includes('@supports not (height: 100svh)'), 'scroll/svh fallback guard missing');
must(css.includes('body[data-screen="fishing"] :is(.bite-callout') && css.includes('body[data-screen="village"] :is(.bottom-nav'), 'fishing/village safe-area selectors missing');
must(css.includes('prefers-reduced-motion: reduce'), 'reduced-motion guard missing');

must(cleanScript.includes("'README.md', 'AI_HANDOFF_CARDVILLE.md'") || cleanScript.includes("'README.md'") && cleanScript.includes("'AI_HANDOFF_CARDVILLE.md'"), 'clean script must whitelist AI_HANDOFF_CARDVILLE.md');
...
console.log('[v21113] UI/UX stability sweep, SVG ban, handoff, and asset guards passed');
