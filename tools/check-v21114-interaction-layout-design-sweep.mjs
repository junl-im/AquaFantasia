...
const fail = (message) => { console.error(`[v21114] ${message}`); process.exit(1); };
const must = (condition, message) => { if (!condition) fail(message); };
const version = '2.1.114';
const cache = 'aqua-fantasia-v2.1.114-interaction-layout-design-sweep';
...
must(pkg.scripts.validate === 'node tools/clean-old-patch-docs.mjs && node tools/validate-clean.mjs && node tools/check-v21114-interaction-layout-design-sweep.mjs', 'validate script must use v2.1.114 guard');
must(data.includes(`APP_VERSION = '${version}'`) && data.includes(cache), 'data.ts version/cache mismatch');
must(sw.includes('v2.1.114') && sw.includes(cache), 'service worker not synced');
must(offline.includes('v2.1.114') && offline.includes('상점/가방/미션/도감'), 'offline page not synced');
must(readme.startsWith('# AquaFantasia v2.1.114') && readme.includes('## v2.1.114 변경사항'), 'README version/history missing');
must(handoff.includes('기준 패키지 버전: `2.1.114`') && handoff.includes('v2.1.114 인터랙션 레이아웃/디자인 스윕'), 'handoff v2.1.114 record missing');

must(main.includes("classList.add('v21113-ui-ux-stability-root')"), 'v2.1.113 root class must remain');
must(main.includes("classList.add('v21114-interaction-layout-design-root')"), 'v2.1.114 root class missing');
must(main.includes("dataset.v21114InteractionLayoutDesign = 'panel-nav-readability-scroll-polish'"), 'v2.1.114 root dataset missing');
must(css.includes('v2.1.114 interaction layout/design sweep'), 'v2.1.114 CSS section missing');
must(css.includes('--v21114-panel-max') && css.includes('--v21114-card-max'), 'v2.1.114 panel/card variables missing');
must(css.includes('scrollbar-gutter: stable') && css.includes('text-wrap: balance'), 'v2.1.114 readability/scroll tokens missing');
must(css.includes('body[data-screen="fishing"] :is(.catch-result-card') && css.includes('body[data-screen="village"] :is(.bottom-nav'), 'fishing/nav safe-area selectors missing');
must(css.includes('@supports not (height: 100svh)') && css.includes('prefers-reduced-motion: reduce'), 'fallback/reduced-motion guards missing');
...
console.log('[v21114] interaction layout/design sweep, SVG ban, handoff, and asset guards passed');
