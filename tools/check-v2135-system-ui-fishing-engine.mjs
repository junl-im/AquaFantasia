...
const fail = (message) => { console.error(`[v2135] ${message}`); process.exit(1); };
...
if (pkg.version !== '2.1.35') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.35' || lock.packages?.['']?.version !== '2.1.35') fail('package-lock.json version mismatch');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.35'", 'APP_VERSION 2.1.35');
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.35-system-ui-fishing-engine', 'v2.1.35 cache name');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.35-system-ui-fishing-engine', 'service worker cache');
assertIncludes('public/offline.html', 'v2.1.35', 'offline badge');
assertIncludes('README.md', '# AquaFantasia v2.1.35', 'README title v2.1.35');
if (fs.existsSync(path.join(root, 'APP_VERSION'))) fail('root APP_VERSION file must not exist in v2.1.35 full package');
...
for (const token of [
  ...
  if (!main.includes(token)) fail(`src/main.ts missing v2.1.35 token: ${token}`);
}
for (const token of [
  ...
  if (!village.includes(token)) fail(`src/villageWorld.ts missing v2.1.35 token: ${token}`);
}
for (const token of [
  'v2135-system-ui-fishing-engine-root',
  '--v2135-aqua-card',
  ...
  if (!styles.includes(token)) fail(`src/styles.css missing v2.1.35 token: ${token}`);
}
for (const token of [
  '마을 배치 엔진',
  '낚시게임을 추가 개선',
...
console.log('[v2135] system UI/fishing engine guard passed');
