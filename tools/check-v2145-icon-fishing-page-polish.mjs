...
const fail = (message) => { console.error(`[v2145] ${message}`); process.exit(1); };
...
if (pkg.version !== '2.1.45') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.45' || lock.packages?.['']?.version !== '2.1.45') fail('package-lock.json version mismatch');
if (!pkg.scripts?.validate?.includes('check-v2145-icon-fishing-page-polish.mjs')) fail('validate script must run v2.1.45 guard');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.45'", 'APP_VERSION 2.1.45');
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.45-icon-fishing-page-polish', 'v2.1.45 cache name');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.45-icon-fishing-page-polish', 'service worker cache');
assertIncludes('public/offline.html', 'v2.1.45', 'offline badge');
assertIncludes('README.md', '# AquaFantasia v2.1.45', 'README title v2.1.45');
...
for (const token of [
  'dataset.v2145IconFishingPagePolish',
  'v2145-icon-fishing-page-polish-root',
  'v2145-runtime-page-shell',
  'v2145-bottom-nav',
  'v2145-fishing-focus-screen',
  'v2145-fishing-stage',
  'v2145-fishing-hud',
  'v2145-sea-lane-card',
  'v2145-fishing-loadout',
  'v2145-fishing-priority-hint',
  'v2145-bite-action',
  'data-v2145-bite-action',
  'v2145-reel-panel',
  'v2145-reel-console',
  'v2145-reel-touch-zone',
  'v2145-cast-button',
  'icon-containment-single-sea-lane-centered-reel-stage',
  'icon-containment-no-stray-sprites-transparent-wrapper',
]) {
  if (!main.includes(token)) fail(`src/main.ts missing v2.1.45 token: ${token}`);
}
...
for (const token of [
  'V2145_ICON_FISHING_PAGE_POLISH_LOCK',
  'V2145_DIAMOND_TOUCH_SCORE_LIMIT = 0.952',
  'bestScore <= V2145_DIAMOND_TOUCH_SCORE_LIMIT',
  'v2145-icon-fishing-page-polish-cautious-tile-no-pixel-shrink',
  'dataset.v2145IconFishingPagePolishLock',
  'const TILE_W = 80',
  'const TILE_H = 40',
]) {
  if (!village.includes(token)) fail(`src/villageWorld.ts missing v2.1.45 tile/placement token: ${token}`);
}
...
for (const token of [
  ...
  '--v2145-gold-cta',
  '.v2145-bottom-nav',
  '.v2145-bottom-nav .v2098-nav-item img',
  '.v2145-runtime-page-shell .runtime-content::before',
  '.shop-card.runtime-shop-card',
  '.v2076-expedition-actions',
  '.v2076-expedition-candidates',
  '.v2145-fishing-hud .hud-chip.region',
  '.v2145-sea-lane-card',
  '.v2145-fishing-loadout',
  '.v2145-fishing-priority-hint',
  '.v2145-bite-action',
  ...
  '.v2145-reel-console',
  '.v2145-reel-touch-zone',
  '.v2145-fishing-focus-screen[data-fishing-phase="reeling"] .v2145-reel-panel:not(.hidden)',
  '--v2145-fishing-reserved',
  'overflow-x: clip',
  'body[data-screen="fishing"] .bottom-nav',
]) {
  if (!styles.includes(token)) fail(`src/styles.css missing v2.1.45 style token: ${token}`);
}

for (const token of [
  '하단 홈/가방/퀘스트/지도 아이콘에서 옆쪽/위쪽 다른 그림 조각',
  '우측 상단 메뉴바는 3중 구조처럼 보이던 외부 배경/테이블 테두리',
  '페이지 뒤쪽에 중복으로 보이던 페이지 제목',
  '항로 조사, 후보지 차트, 출항 후보지 카드',
  '상점 아이템 카드는 글 영역과 가격 버튼 영역을 분리',
  '바다물길 표기를 하나로 정리',
  '릴링 본 게임 패널이 왼쪽 화면 밖으로 밀려나지 않도록 중앙 고정',
  '다이아몬드 터치 점수를 0.952',
  '`npm ci`가 만든 `node_modules`는 작업환경 의존성으로 허용',
]) {
  if (!readme.includes(token)) fail(`README.md missing v2.1.45 note: ${token}`);
}

// Icon sprites used by the bottom dock must have transparent padding at every edge so adjacent art cannot peek in.
for (const file of ['nav_village.png', 'nav_bag.png', 'nav_quest.png', 'nav_map.png']) {
  ...
  if (raw.length < 1000) fail(`${rel} looks unexpectedly tiny or empty`);
  const sizeSig = raw.slice(16, 24).toString('hex');
  if (!sizeSig) fail(`${rel} PNG header unreadable`);
}
...
console.log('[v2145] icon containment, page title, shop, fishing, HUD/menu/tile guard passed');
