...
if (pkg.version !== '9.0.0') fail('package version mismatch');
if (!data.includes("APP_VERSION = '9.0.0'")) fail('APP_VERSION is not 9.0.0');
if (!data.includes('aqua-fantasia-v9.0.0-webgl-underwater-cute-runtime')) fail('CACHE_NAME mismatch');
console.log('[validate-clean] Aqua Fantasia v9.0.0 clean single README package OK');
console.log(JSON.stringify({ ok:true, version:'9.0.0', files: files.length }, null, 2));
