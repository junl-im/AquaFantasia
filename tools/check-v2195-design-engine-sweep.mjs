...
const fail = (msg) => { console.error(`[v2195] ${msg}`); process.exit(1); };
...
if (pkg.version !== '2.1.95') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.95' || lock.packages?.['']?.version !== '2.1.95') fail('package-lock.json version mismatch');
if (!pkg.scripts?.validate?.includes('check-v2195-design-engine-sweep.mjs')) fail('validate script must run v2.1.95 guard');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.95'");
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.95-premium-design-engine-sweep');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.95-premium-design-engine-sweep');
assertIncludes('public/offline.html', 'v2.1.95');
assertIncludes('README.md', '# AquaFantasia v2.1.95');
assertIncludes('README.md', '## v2.1.95 변경사항');
...
for (const token of [
  ...
  'installV2195PremiumDesignEngineSweepPass',
  'dataset.v2195PremiumDesignEngineSweep',
  'v2195-premium-design-engine-root',
  'v2195-opening-cinematic',
  'v2195-opening-video',
  'native-controls-play-mark-suppressed-until-first-frame',
  'v2195-fishing-slim-design-screen',
  'ultra-slim-two-half-loadout-premium-layout',
  'v2195-fishing-loadout',
  'v2195-loadout-cell',
  'v2195-loadout-icon',
  'micro-inline-rod-bait',
  'v2195-loadout-copy',
  'v2195-ocean-glass-card'
...
if (!renderOpening.includes('v2195-opening-video')) fail('opening render block must contain v2195 video');
...
if (!/video\.controls\s*=\s*false/.test(main) || !/removeAttribute\('controls'\)/.test(main)) fail('opening video controls must be disabled and removed');
...
for (const token of [
  '/* v2.1.95: premium design engine sweep',
  'html.v2195-premium-design-engine-root',
  '.v2195-opening-video::-webkit-media-controls-start-playback-button',
  '.v2195-opening-cinematic::before',
  '.v2195-fishing-loadout',
  'grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) !important;',
  '.v2195-loadout-cell',
  'grid-template-columns: var(--v2195-loadout-icon) minmax(0, 1fr) !important;',
  '--v2195-loadout-icon: 12px;',
  '.v2195-loadout-copy',
  '.v2195-ocean-glass-card'
...
console.log('[v2195] premium design engine, opening shell, slim loadout, player direction, package checks passed');
