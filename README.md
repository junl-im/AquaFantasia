# AquaFantasia v2.1.101

## v2.1.101 변경사항

- v2.1.100 마일스톤 위에 누적된 디자인 보정 레이어를 더 가볍게 정리하고, 활성 화면 기준으로만 카드/버튼/입력창 보정이 돌도록 연속성 패스를 추가했습니다.
- 오프닝 영상은 첫 실제 프레임 전 브라우저 네이티브 플레이 마크와 중간 UI가 보이지 않도록 first-frame mask와 video-only 계약을 유지/보강했습니다. `poster` 정지 이미지는 계속 사용하지 않습니다.
- 낚시 준비 화면의 낚싯대/미끼 정보는 상단 얇은 2분할 바를 유지하면서 아이콘을 마이크로 크기로 제한하고 텍스트 우선 가독성, 좌우 안전영역, 자동 높이 보정을 보강했습니다.
- 입질/실전/결과 단계에서는 장비바, 게임 설명, 최근 포획, 게이지/릴 콘솔 잔상이 서로 겹치지 않도록 cleanup과 compact result guard를 다시 정리했습니다.
- visualViewport, reduced-motion, image lazy/async/no-drag, layout/paint containment를 유지하고 v2.1.101 전용 검증 스크립트를 추가했습니다.

# AquaFantasia v2.1.100
