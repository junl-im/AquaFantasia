...
if (!state.includes("APP_VERSION = '5.4.0'") || !state.includes('aqua_v5.4') || !state.includes('sellFish') || !state.includes('buyShopItem')) fail('state v5.4 shop/sell markers missing');
...
if (!index.includes('v54-result-shop-polish.js') || !index.includes("const APP_VERSION = '5.4.0'")) fail('index v5.4 marker missing');
if (!sw.includes('aqua-fantasia-v5.4.0') || !sw.includes('./src/runtime/v54-result-shop-polish.js') || !sw.includes('./src/systems/shop.js')) fail('service worker v5.4 cache entries missing');
