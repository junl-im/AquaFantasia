...
const fail = (msg) => { console.error(`[v2191] ${msg}`); process.exit(1); };
...
if (pkg.version !== '2.1.91') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.91' || lock.packages?.['']?.version !== '2.1.91') fail('package-lock.json version mismatch');
if (!pkg.scripts?.validate?.includes('check-v2191-opening-fishing-design-engine.mjs')) fail('validate script must run v2.1.91 guard');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.91'");
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.91-opening-fishing-design-engine');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.91-opening-fishing-design-engine');
assertIncludes('public/offline.html', 'v2.1.91');
assertIncludes('README.md', '# AquaFantasia v2.1.91');
...
for (const token of [
  ...
  'installV2191OpeningFishingDesignEnginePass',
  'dataset.v2191OpeningFishingDesignEngine',
  'v2191-opening-fishing-design-root',
  'v2191-opening-cinematic',
  'v2191-opening-video',
  'video-only-no-status-no-vignette-no-skip',
  'v2191-opening-hide-village-chrome',
  'v2191-fishing-design-screen',
  'smaller-rod-bait-icons-readable-shelf',
  'v2191-loadout-cell',
  'v2191-loadout-icon',
  'rod-bait-icon-reduced',
  'v2191-loadout-copy',
  'v2191-bite-callout',
  'v2191-result-card',
...
for (const token of [
  '/* v2.1.91: opening video-only contract',
  'html.v2191-opening-fishing-design-root',
  '.v2191-opening-cinematic',
  '.v2191-opening-video',
  'v2191-opening-video-only-contract',
  '.v2191-loadout-cell',
  'grid-template-columns: var(--v2191-loadout-track) minmax(0, 1fr) !important;',
  '.v2191-loadout-icon',
  'width: var(--v2191-loadout-icon) !important;',
  '.v2191-bite-callout',
  'top: var(--v2191-bite-top) !important;',
  '.v2191-result-card',
]) if (!styles.includes(token)) fail(`src/styles.css missing ${token}`);
for (const token of [
  '## v2.1.91 변경사항',
  '오프닝 영상 단독 계약',
  '낚시 준비 화면의 낚싯대/미끼 아이콘',
  '프리미엄 오션 글래스',
...
console.log('[v2191] opening video-only, smaller fishing equipment icons, premium design, player direction, package checks passed');
