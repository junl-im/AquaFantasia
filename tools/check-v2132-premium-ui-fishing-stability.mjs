...
if (pkg.version !== '2.1.32') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.32' || lock.packages?.['']?.version !== '2.1.32') fail('package-lock.json version mismatch');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.32'", 'APP_VERSION 2.1.32');
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.32-premium-ui-fishing-stability', 'v2.1.32 cache name');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.32-premium-ui-fishing-stability', 'service worker cache');
assertIncludes('public/offline.html', 'v2.1.32', 'offline badge');
assertIncludes('README.md', '# AquaFantasia v2.1.32', 'README title v2.1.32');
if (fs.existsSync(path.join(root, 'APP_VERSION'))) {
  const staleAppVersion = read('APP_VERSION').trim();
  if (staleAppVersion && staleAppVersion !== pkg.version) {
    console.warn(`[v2132] stale root APP_VERSION file ignored: ${staleAppVersion}`);
  }
}
...
console.log('[v2132] premium UI/fishing stability guard passed');
