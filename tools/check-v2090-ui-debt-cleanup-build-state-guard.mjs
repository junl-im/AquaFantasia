...
if (pkg.version !== '2.0.90') fail('package.json version must be 2.0.90');
...
if (!data.includes(`aqua-fantasia-v${pkg.version}-ui-debt-cleanup-build-state-guard`)) fail('cache name mismatch');
if (!sw.includes(`aqua-fantasia-v${pkg.version}-ui-debt-cleanup-build-state-guard`)) fail('service worker cache mismatch');
