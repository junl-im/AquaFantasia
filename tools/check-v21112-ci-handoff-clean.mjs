...
const fail = (message) => { console.error(`[v21112] ${message}`); process.exit(1); };
const must = (condition, message) => { if (!condition) fail(message); };
const version = '2.1.112';
const cache = 'aqua-fantasia-v2.1.112-ci-handoff-clean';
...
must(pkg.scripts.validate === 'node tools/clean-old-patch-docs.mjs && node tools/validate-clean.mjs && node tools/check-v21112-ci-handoff-clean.mjs', 'validate script must use the v2.1.112 handoff clean guard');
must(data.includes(`APP_VERSION = '${version}'`) && data.includes(cache), 'data.ts version/cache mismatch');
must(sw.includes('v2.1.112') && sw.includes(cache), 'service worker not synced');
must(offline.includes('v2.1.112') && offline.includes('AI_HANDOFF_CARDVILLE.md 삭제 재발 방지'), 'offline page not synced');
must(readme.startsWith('# AquaFantasia v2.1.112') && readme.includes('## v2.1.112 변경사항'), 'README version/history missing');
must(handoff.includes('기준 패키지 버전: `2.1.112`') && handoff.includes('GitHub Actions validate 실패 원인') && handoff.includes('AI_HANDOFF_CARDVILLE.md 삭제 재발 방지'), 'handoff CI failure record missing');
...
must(cleanScript.includes('isAllowedRootMarkdown') && cleanScript.includes('!rel.includes'), 'clean script must only preserve required root markdown files intentionally');
must(validateClean.includes("requiredMarkdown") && validateClean.includes("AI_HANDOFF_CARDVILLE.md") && validateClean.includes('markdown must be exactly README.md and AI_HANDOFF_CARDVILLE.md'), 'validate-clean must require handoff markdown');
...
console.log('[v21112] CI handoff clean guard, SVG ban, and asset policy checks passed');
