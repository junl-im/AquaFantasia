...
class AquaFantasiaGame {
  ...
  private renderVillage(): void {
    ...
    root.innerHTML = `
      ...
      <header class="v2-village-hud glass-card aqua-card-surface aqua-card-hud v2084-compact-hud v2085-slim-village-hud v2086-slim-village-hud v2087-slim-village-hud v2088-character-hud v2089-village-hud" data-v2053-profile-open aria-label="마을 상태">
        <button class="v2-profile-chip v2017-profile-button v2084-profile-chip v2085-village-profile-chip v2086-village-profile-chip v2087-village-profile-chip v2088-profile-chip v2089-profile-chip" type="button" data-v2017-profile aria-haspopup="dialog" aria-label="내 캐릭터 열기"><span data-v2-level>Lv.${this.playerLevel()}</span><strong data-v2041-profile-chip-name>${playerNameHtml}</strong><em>루미나 베이</em></button>
      ...
      <section class="v2050-expedition-board v2051-expedition-mini v2075-expedition-dock glass-card aqua-card-mini-bar v2088-expedition-board v2089-expedition-board" aria-label="다른 섬 개척 준비 보드"><button type="button" class="v2051-loop-toggle v2075-expedition-toggle v2088-expedition-toggle v2089-expedition-toggle" aria-label="개척 정보 펼치기" aria-expanded="false"><span>개척</span><strong>${expansionStats.progress}%</strong><em>${expansionStats.phaseLabel}</em></button><div class="v2051-loop-body v2088-expedition-body v2089-expedition-body" aria-hidden="true">${this.islandExpansionBoardMarkup()}<button type="button" class="v2055-loop-close v2059-loop-close v2089-ui-close v2089-expedition-close" data-v2055-loop-close aria-label="정보 닫기">×</button></div></section>
      ...
      <section class="v2017-character-panel" data-v2017-character-panel aria-hidden="true" role="dialog" aria-modal="true" aria-label="내 캐릭터">
        ...
        <article class="v2017-character-card glass-card aqua-card-surface aqua-card-dialog v2088-character-card">
          <button type="button" class="v2017-character-close v2089-ui-close" data-v2017-character-close aria-label="내 캐릭터 닫기">×</button>
      ...
      <section class="v203-interior-panel v206-interior-panel v2088-interior-panel" aria-live="polite" aria-hidden="true">
        ...
        <article class="v203-interior-card v206-interior-card glass-card aqua-card-surface aqua-card-dialog v2088-interior-card">
          <button type="button" class="v2059-screen-close v2059-panel-close v2088-dialog-close v2089-ui-close" data-v203-interior-close aria-label="건물 정보 닫기">×</button>
      ...
      <aside class="v2-build-tray glass-card v2088-build-card" aria-label="건물 설치 모드" role="dialog" aria-modal="true">
        <div class="v2-build-title"><strong>설치모드</strong><button type="button" class="v2059-screen-close v2059-build-close v2088-dialog-close v2089-ui-close" data-village-build-close aria-label="건설창 닫기">×</button></div>
    ...
    root.querySelectorAll<HTMLButtonElement>('.v2051-loop-toggle').forEach((btn) => btn.addEventListener('click', () => {
      ...
      root.querySelectorAll<HTMLElement>('.v2051-loop-mini.open, .v2051-expedition-mini.open').forEach((other) => {
        if (other !== panel) {
          ...
          other.querySelector<HTMLElement>('.v2051-loop-toggle')?.setAttribute('aria-expanded', 'false');
        }
      });
      panel.classList.toggle('open', willOpen);
      root.classList.toggle('v2081-expedition-panel-open', willOpen);
      root.classList.toggle('v2082-expedition-panel-open', willOpen);
      ...
      document.body.classList.remove('v2084-popup-open', 'v2086-modal-open', 'v2087-modal-open', 'v2088-modal-open');
      root.classList.toggle('v2088-expedition-open', willOpen);
      root.classList.toggle('v2089-expedition-open', willOpen);
      document.body.classList.toggle('v2088-expedition-open', willOpen);
      document.body.classList.toggle('v2089-expedition-open', willOpen);
      ...
      panel.querySelector<HTMLElement>('.v2051-loop-body')?.classList.toggle('v2088-expedition-body-open', willOpen);
      panel.querySelector<HTMLElement>('.v2051-loop-body')?.classList.toggle('v2089-expedition-body-open', willOpen);
      panel.querySelector<HTMLElement>('.v2051-loop-body')?.setAttribute('aria-hidden', String(!willOpen));
      panel.querySelector<HTMLElement>('.v2051-loop-toggle')?.setAttribute('aria-expanded', String(willOpen));
    }));
    root.querySelectorAll<HTMLButtonElement>('[data-v2055-loop-close]').forEach((btn) => btn.addEventListener('click', (ev) => {
      ev.stopPropagation();
      const panel = btn.closest<HTMLElement>('.v2051-loop-mini, .v2051-expedition-mini');
      panel?.classList.remove('open');
      root.classList.remove('v2081-expedition-panel-open', 'v2082-expedition-panel-open', 'v2084-popup-open', 'v2085-modal-open', 'v2086-modal-open', 'v2087-modal-open', 'v2088-modal-open', 'v2088-expedition-open', 'v2089-modal-open', 'v2089-expedition-open');
      document.body.classList.remove('v2081-expedition-panel-open', 'v2082-expedition-panel-open', 'v2084-popup-open', 'v2085-modal-open', 'v2086-modal-open', 'v2087-modal-open', 'v2088-modal-open', 'v2088-expedition-open', 'v2089-modal-open', 'v2089-expedition-open');
      panel?.querySelector<HTMLElement>('.v2051-loop-body')?.classList.remove('v2081-expedition-body', 'v2082-expedition-body', 'v2088-expedition-body-open', 'v2089-expedition-body-open');
  ...
  private createRuntimeMenuScreen(active: Exclude<Screen, 'login' | 'fishing'>, title: string, subtitle: string): HTMLElement {
    ...
    root.innerHTML = `
      ...
      <div class="aqua-modal-dim v2081-menu-dim v2082-menu-dim v2084-aqua-modal-dim v2087-aqua-modal-dim v2088-aqua-dim" aria-hidden="true"></div>
      ${active === 'village' ? '' : '<button type="button" class="v2059-screen-close v2059-menu-close v2077-menu-close v2082-menu-close v2084-close-button v2087-close-button v2088-close-button v2089-ui-close v2089-menu-close" data-v2059-menu-close data-v2077-close aria-label="메뉴 닫기">×</button>'}
      <img class="runtime-bg-character" src="${ASSET.player}" alt="" aria-hidden="true" loading="eager" />
      <header class="runtime-hud aqua-card-surface aqua-card-hud v2082-runtime-hud v2084-runtime-hud v2085-slim-runtime-hud v2086-slim-runtime-hud v2087-slim-runtime-hud v2088-runtime-hud v2089-runtime-hud" aria-label="플레이어 HUD">
        <button type="button" class="runtime-hud-mascot v2082-hud-mascot v2084-hud-avatar v2085-hud-avatar v2086-hud-avatar v2087-hud-avatar v2088-hud-avatar v2089-hud-avatar" data-v2084-profile-shortcut aria-label="내 캐릭터 정보로 돌아가기"><img src="./assets/v203/portraits/player_happy.png" alt="" /></button>
        <div class="runtime-title v2082-hud-title v2084-hud-title v2085-hud-title v2086-hud-title v2087-hud-title v2088-hud-title v2089-hud-title"><span>Lv.${this.playerLevel()} · ${this.escapeHtml(this.playerName())}</span><strong>${title}</strong><em>루미나 베이</em></div>
      ...
      <div class="runtime-content aqua-card-surface aqua-card-page v2080-scroll-stack v2082-menu-content v2084-menu-content v2085-menu-content v2086-menu-content v2082-${active}-content v2084-${active}-content v2085-${active}-content v2086-${active}-content v2087-menu-content v2088-menu-content v2089-menu-content v2087-${active}-content v2089-${active}-content" data-v2082-scroll-root="true" data-v2084-scroll-root="true" data-v2085-scroll-root="true" data-v2086-scroll-root="true" data-v2087-scroll-root="true" data-v2088-scroll-root="true" data-v2089-scroll-root="true"></div>`;
