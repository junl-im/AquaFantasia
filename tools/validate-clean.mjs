...
if (!data.includes("APP_VERSION = '8.4.0'")) fail('APP_VERSION is not 8.4.0');
if (!data.includes('aqua-fantasia-v8.4.0-fishing-fullscreen-ui-polish')) fail('CACHE_NAME mismatch');
const pkg = JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8'));
if (pkg.version !== '8.4.0') fail('package version mismatch');
...
console.log('[validate-clean] Aqua Fantasia v8.4.0 fullscreen fishing ui polish OK');
console.log(JSON.stringify({ ok:true, version:'8.4.0', files: walk(root).length }, null, 2));
