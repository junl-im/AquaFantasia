...
if (pkg.version !== '2.0.92') fail('package.json version must be 2.0.92');
...
if (!data.includes(`aqua-fantasia-v${pkg.version}-ui-canonical-stable-shell`)) fail('cache name mismatch');
if (!sw.includes(`aqua-fantasia-v${pkg.version}-ui-canonical-stable-shell`)) fail('service worker cache mismatch');
