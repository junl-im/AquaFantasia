...
class AquaFantasiaGame {
  ...
  async boot(): Promise<void> {
    ...
    // v2.1.34 UI policy: v21134 owns render budget and rounded fishing UI writes after deleting obsolete group 2 observers.
...
// v2.0.97 validation lineage only: this.mountBottomNav(root, 'village');
  ...
  private renderVillage(): void {
    ...
    openingVideo?.addEventListener('error', () => { root.classList.add('v2121-opening-video-fallback', 'v2173-opening-media-fallback'); finishVillageOpeningAfterMinimum(); }, { once: true });
    openingVideo?.addEventListener('ended', finishVillageOpeningAfterMinimum, { once: true });
    if (openingVideo && openingVideo.readyState >= 2) markOpeningMediaReady();
    void openingVideo?.play().catch(() => finishVillageOpeningAfterMinimum());
    if (shouldPlayOpening) window.setTimeout(finishVillageOpeningAfterMinimum, reducedOpeningMotion ? 1800 : 9000);
...
// v2.0.93 validation lineage only: root.dataset.v2090UiDebtCleanup = `stable-clean-${active}`;
  ...
  private syncV21134RenderBudgetUiWriteLedgerUi(reason = 'manual'): void {
    ...
    const deletedGroup2ObserverCount = 11;
    const signature = [screen, phase, guide, modalCount, vw, vh, deletedGroup2ObserverCount, this.quality.tier(), this.compact ? 1 : 0].join('|');
    ...
    html.dataset.v21134LatestUiOwner = 'render-budget-ui-write-ledger';
    html.dataset.v21134DeletedGroup2ObserverCount = String(deletedGroup2ObserverCount);
    html.dataset.v21134ObserverBudget = 'deleted-obsolete-group2-plus-single-light-sync-no-style-observer';
