...
if (pkg.version !== '2.1.31') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.31' || lock.packages?.['']?.version !== '2.1.31') fail('package-lock.json version mismatch');
assertIncludes('APP_VERSION', '2.1.31', 'APP_VERSION file 2.1.31');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.31'", 'APP_VERSION 2.1.31');
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.31-motion-ui-fishing-build-guard', 'v2.1.31 cache name');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.31-motion-ui-fishing-build-guard', 'service worker cache');
assertIncludes('public/offline.html', 'v2.1.31', 'offline badge');
assertIncludes('README.md', '# AquaFantasia v2.1.31', 'README title v2.1.31');
...
console.log('[v2131] motion/ui/fishing/build guard passed');
