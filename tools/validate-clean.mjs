...
const required = [
  ...
  'public/assets/ui/button_primary.png', 'public/assets/ui/button_soft.png', 'public/assets/ui/dex_panel_reference_25d.png', 'public/assets/screens/start_screen_reference.webp', 'public/assets/screens/start_screen_clean_v690.webp', 'public/assets/screens/start_screen_clean_v710.webp', 'public/assets/screens/start_screen_clean_v720.webp', 'public/assets/ui/v710_button_yellow.png', 'public/assets/ui/v710_button_blue.png', 'public/assets/ui/v710_combo_ribbon.png', 'public/assets/ui/v720_toggle_on.png', 'public/assets/ui/v720_toggle_off.png', 'public/assets/ui/v720_panel_clean.png', 'public/assets/ui/button_cast_clean.png', 'public/assets/ui/button_cast_yellow_clean.png',
...
if (!data.includes("APP_VERSION = '7.2.0'")) fail('APP_VERSION is not 7.2.0');
...
if (!sw.includes('aqua-fantasia-v7.2.0-total-ui-transparent-dex')) fail('service worker cache version mismatch');
...
if (pkg.version !== '7.2.0') fail('package version mismatch');
...
console.log('[validate-clean] Aqua Fantasia v7.2.0 total UI and transparent fish dex OK');
console.log(JSON.stringify({ ok: true, version: '7.2.0', files: files.length, atlasFrames: Object.keys(atlas.frames).length }, null, 2));

// v7.2.0 additional assertions
for (const token of ['start_screen_clean_v720.webp','v720-keep-toggle','배경 없는 투명 2.5D']) { if (!main.includes(token) && !data.includes(token) && !fs.readFileSync(path.join(root, 'src/styles.css'), 'utf8').includes(token)) fail(`missing v7.2 token ${token}`); }
