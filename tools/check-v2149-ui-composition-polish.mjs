...
const fail = (message) => { console.error(`[v2149] ${message}`); process.exit(1); };
...
if (pkg.version !== '2.1.49') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.49' || lock.packages?.['']?.version !== '2.1.49') fail('package-lock.json version mismatch');
if (!pkg.scripts?.validate?.includes('check-v2149-ui-composition-polish.mjs')) fail('validate script must run v2.1.49 guard');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.49'", 'APP_VERSION 2.1.49');
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.49-ui-composition-polish', 'v2.1.49 cache name');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.49-ui-composition-polish', 'service worker cache');
assertIncludes('public/offline.html', 'v2.1.49', 'offline badge');
assertIncludes('README.md', '# AquaFantasia v2.1.49', 'README title');
...
for (const token of [
  'dataset.v2149UiCompositionPolish',
  'v2149-ui-composition-polish-root',
  'v2149-world-controls',
  'data-v2149-primary-order="plus-minus-build-center-shop-sail"',
  'v2149-runtime-page-shell',
  'data-v2149-page-title-clean="true"',
  'v2149-bottom-nav',
  'transparent-wrapper-single-cell-clean-safe-icons',
  'v2149-fishing-focus-screen',
  'v2149-fishing-stage',
  'v2149-fishing-hud',
  'v2149-sea-lane-card',
  'v2149-fishing-loadout',
  'v2149-fishing-coach',
  'v2149-bite-action',
  'data-v2149-bite-action',
  'v2149-reel-panel',
  'v2149-reel-console',
  'v2149-reel-touch-zone',
  'v2149-cast-button',
]) if (!main.includes(token)) fail(`src/main.ts missing v2.1.49 token: ${token}`);
...
for (const token of [
  'V2149_UI_COMPOSITION_POLISH_LOCK',
  'V2149_DIAMOND_TOUCH_SCORE_LIMIT = 0.944',
  'bestScore <= V2149_DIAMOND_TOUCH_SCORE_LIMIT',
  'v2149-ui-composition-polish-touch-cautious-no-tile-shrink',
  'dataset.v2149UiCompositionPolishLock',
  'const TILE_W = 80',
  'const TILE_H = 40',
]) if (!village.includes(token)) fail(`src/villageWorld.ts missing v2.1.49 tile token: ${token}`);
...
for (const token of [
  ...
  '.v2149-world-controls',
  '.v2149-bottom-nav',
  '.v2149-bottom-nav .v2098-nav-item img',
  '.runtime-content[data-v2149-page-title-clean="true"]::before',
  '.shop-card.runtime-shop-card',
  '.v2076-expedition-candidates',
  '.v2149-fishing-hud .hud-chip.region',
  '.v2149-sea-lane-card',
  '.v2149-fishing-loadout',
  '.v2149-fishing-coach',
  '.v2149-bite-action',
  ...
  '.v2149-reel-console',
  '.v2149-reel-touch-zone',
  '.v2149-fishing-focus-screen[data-fishing-phase="reeling"] .v2149-reel-panel:not(.hidden)',
  'left: calc(var(--v2149-safe-left) + 10px) !important;',
  'right: calc(var(--v2149-safe-right) + 10px) !important;',
  'body[data-screen="fishing"] .bottom-nav',
]) if (!styles.includes(token)) fail(`src/styles.css missing v2.1.49 style token: ${token}`);

for (const token of [
  '우측 상단 마을 메뉴는 뒤쪽 배경 프레임을 계속 제거하고',
  '투명 여백을 더 확보한 128x128 PNG',
  'data-v2149-page-title-clean',
  '상점 아이템 카드는 글 영역과 골드 구매 버튼 영역을 분리',
  '낚시 화면은 준비 단계와 릴링 단계를 더 강하게 분리',
  '다이아몬드 터치 점수만 `0.944`',
  'node_modules가 Git에 추적되거나 full/patch ZIP에 들어가는 것은 계속 차단',
]) if (!readme.includes(token)) fail(`README.md missing v2.1.49 note: ${token}`);
...
console.log('[v2149] UI composition polish guard passed');
