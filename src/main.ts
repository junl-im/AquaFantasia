...
async function boot() {
  ...
  await loadAquaAtlas('/assets/atlas/aqua_fishing_v47.atlas.json').catch(console.warn);
  if (host) {
    const stage = new AquaPixiStage();
    await stage.init({ canvasHost: host, atlasUrl: '/assets/atlas/aqua_fishing_v47.webp', atlasJsonUrl: '/assets/atlas/aqua_fishing_v47.atlas.json', quality, dprCap: quality === 'lite' ? 1.25 : 2 });
  }
...
boot().catch((error) => console.error('[Aqua v4.7 renderer boot]', error));
