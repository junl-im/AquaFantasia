...
const required = [
  ...
  'public/assets/v88/characters/fisher_boat_ultra.png','public/assets/v88/icons/village.png','public/assets/v88/ui/bottom_nav_hd.png','public/assets/v88/ui/recent_panel_hd.png','public/assets/v88/ui/reel_panel_hd.png','public/assets/v88/ui/tab_blue_hd.png','public/assets/v88/ui/tab_gold_hd.png'
];
...
if (!data.includes("APP_VERSION = '8.8.0'")) fail('APP_VERSION is not 8.8.0');
if (!data.includes('aqua-fantasia-v8.8.0-browser-fullscreen-runtime-ui')) fail('CACHE_NAME mismatch');
const pkg = JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8'));
if (pkg.version !== '8.8.0') fail('package version mismatch');
...
if (!guard.includes('requestFullscreen')) fail('safe fullscreen helper missing');
if (!guard.includes('metrics.hostileInApp')) fail('fullscreen must be disabled in Kakao/in-app shells');
...
console.log('[validate-clean] Aqua Fantasia v8.8.0 clean single README package OK');
console.log(JSON.stringify({ ok:true, version:'8.8.0', files: walk(root).length }, null, 2));
