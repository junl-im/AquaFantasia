...
- 모든 런타임 메뉴 페이지에 `v2077` 아쿠아 카드 최종 정리 레이어를 추가해 가방/지도/퀘스트/상점/장비/도감 카드와 버튼 톤을 캐릭터 정보창 계열로 다시 맞췄습니다.
- 가방을 포함한 긴 메뉴 페이지에서 카드 영역을 드래그했을 때 스크롤이 멈추는 문제를 줄이도록 스크롤 타깃 감지 대상을 확장하고, 버튼 터치 후 닫기 `×`가 먹지 않을 수 있던 클릭 억제 로직을 수정했습니다.
- 모든 메뉴 닫기 `×` 버튼을 더 큰 원형 아쿠아 버튼으로 통일하고 `data-v2077-close` 보조 닫기 훅을 추가해 닫기 클릭 안정성을 보강했습니다.
- 이전 `v2.0.75` 최종 배치 오버라이드가 개척 버튼을 하단으로 밀던 문제를 `v2077` 최종 오버라이드로 재정렬해, `개척` 얇은 바가 HUD 아래쪽에 고정되도록 수정했습니다.
- `개척` 팝업은 좌우 safe-area 기준으로 다시 고정해 화면 밖으로 벗어나지 않게 하고, 내부 후보지/실행 버튼 목록은 세로 스크롤 가능한 아쿠아 카드로 정리했습니다.
- `루프` 버튼은 계속 제거 상태를 유지하고, 성장 루프 정보는 `개척` 패널 안에만 통합 표시되도록 중복 UI 회귀를 검증합니다.
- `v2.0.77` 전용 검증 스크립트 `check-v2077-menu-ui-cleanup.mjs`를 추가하고 validate에 연결했습니다.
- `package.json`, `package-lock.json`, `APP_VERSION`, service worker cache, offline badge, README를 `2.0.77`로 동기화했습니다.
