...
class AquaFantasiaGame {
  ...
  private clear(): void {
    ...
    document.body.classList.remove('v2032-character-panel-open', 'v2033-character-panel-open', 'v2034-character-panel-open', 'v2035-character-panel-open', 'v2036-character-panel-open');
  ...
  private async renderFishing(): Promise<void> {
    ...
    root.innerHTML = `
      ...
      <div class="fishing-stage v2028-fishing-stage v2030-fishing-stage v2031-fishing-stage v2032-fishing-stage v2033-fishing-stage v2034-fishing-stage v2035-fishing-stage v2036-fishing-stage" id="fishingStage">
      ...
      <div class="fishing-hud v205-fishing-hud v2028-fishing-hud v2029-fishing-hud v2030-fishing-hud v2031-fishing-hud v2032-fishing-hud v2033-fishing-hud v2034-fishing-hud v2035-fishing-hud v2036-fishing-hud" aria-label="플레이어 정보">
      ...
      <section class="recent-catch-strip v205-recent-catch v2029-recent-catch-card v2030-recent-catch-card v2031-recent-catch-card v2032-recent-catch-card v2033-recent-catch-card v2034-recent-catch-card v2035-recent-catch-card v2036-recent-catch-card" aria-label="최근 포획">
        ${this.recentCatchMarkup()}
      </section>
      <div class="reel-panel glass-card hidden v205-reel-panel v2028-reel-panel v2029-reel-panel v2030-reel-panel v2031-reel-panel v2032-reel-panel v2033-reel-panel v2034-reel-panel v2035-reel-panel v2036-reel-panel" id="reelPanel">
  ...
  private mountBottomNav(root: HTMLElement, active: Screen): void {
    ...
    const v13 = false;
    nav.className = 'bottom-nav fixed-root-nav v2016-safe-dock-nav v2026-unified-dock-nav v2027-aqua-dock-nav v2029-home-dock-nav v2031-identical-dock-nav v2032-identical-dock-nav v2033-identical-dock-nav v2034-identical-dock-nav v2035-identical-dock-nav v2036-identical-dock-nav';
    ...
    const dockInlineStyles: Array<[string, string]> = [
      ['position', 'fixed'], ['left', 'auto'], ['right', 'var(--v2036-dock-right, max(10px, env(safe-area-inset-right)))'],
      ['bottom', 'var(--v2036-dock-bottom, calc(max(14px, env(safe-area-inset-bottom)) + 6px))'], ['width', 'auto'], ['height', 'auto'],
      ['min-width', '0'], ['max-width', 'calc(100vw - 16px)'], ['display', 'grid'],
      ['grid-template-columns', 'repeat(3, var(--v2036-dock-button, 52px))'],
      ['grid-template-rows', 'repeat(2, var(--v2036-dock-button, 52px))'], ['gap', 'var(--v2036-dock-gap, 4px)'],
  ...
  private showResultCard(reward: number): void {
    ...
    card.className = `catch-result-card v930-result v2021-result-card v2036-result-card rarity-${this.activeFish.rarity.toLowerCase()}`;
