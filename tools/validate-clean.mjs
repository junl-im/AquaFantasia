...
const required = [
  ...
  'public/sw.js','public/manifest.webmanifest','public/assets/screens/start_screen_clean_v740.webp','public/assets/ui/v740_button_gold.png','public/assets/ui/v740_keep_on.png',
...
if (!data.includes("APP_VERSION = '7.4.0'")) fail('APP_VERSION is not 7.4.0');
if (!data.includes('aqua-fantasia-v7.4.0-user-asset-pack')) fail('CACHE_NAME mismatch');
...
for (const token of ['installBackNavigationGuard','showBiteCallout','requestHardPortraitLock','inapp-css-only','start_screen_clean_v740']) if (!main.includes(token) && !fs.readFileSync(path.join(root,'src/core/PortraitGuard.ts'),'utf8').includes(token)) fail(`missing runtime token ${token}`);
if (!ok) process.exit(1);
console.log('[validate-clean] Aqua Fantasia v7.4.0 user asset pack OK');
console.log(JSON.stringify({ ok:true, version:'7.4.0', files: walk(root).length }, null, 2));
