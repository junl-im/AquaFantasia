...
const fail = (msg) => { console.error(`[v2186] ${msg}`); process.exit(1); };
...
if (pkg.version !== '2.1.86') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.86' || lock.packages?.['']?.version !== '2.1.86') fail('package-lock.json version mismatch');
if (!pkg.scripts?.validate?.includes('check-v2186-system-quality-ui-engine-polish.mjs')) fail('validate script must run v2.1.86 guard');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.86'");
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.86-system-quality-ui-engine-polish');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.86-system-quality-ui-engine-polish');
assertIncludes('public/offline.html', 'v2.1.86');
assertIncludes('README.md', '# AquaFantasia v2.1.86');
assertIncludes('README.md', '## v2.1.86 변경사항');
...
for (const token of [
  ...
  'installV2186SystemQualityUiEnginePolishPass',
  'dataset.v2186SystemQualityUiEnginePolish',
  'v2186-system-quality-ui-engine-root',
  'fishing-shelf-bite-result-cleanup-performance-polish',
  'v2186-fishing-quality-screen',
  'measured-loadout-bite-result-no-stale-gauge',
  'v2186-sea-lane-card',
  'v2186-fishing-loadout',
  'v2186-loadout-cell',
  'v2186-loadout-copy',
  'v2186-readable-loadout-text',
  'v2186-bite-callout',
  'v2186-focused-phase-no-overlap',
  'v2186-gauge-lifecycle-cleanup',
  'v2186-result-card',
  'v2186-result-rarity-compact',
  'v2186-result-impact',
  'v2186-village-stability-screen',
  'v2186-hud-frontier-gap',
]) if (!main.includes(token)) fail(`src/main.ts missing ${token}`);
for (const token of [
  '/* v2.1.86: system quality UI/engine polish',
  'html.v2186-system-quality-ui-engine-root',
  'body[data-screen="fishing"] .v2186-sea-lane-card',
  '.v2186-fishing-loadout',
  '.v2186-loadout-cell',
  '.v2186-loadout-copy',
  '.v2186-readable-loadout-text',
  ...
  '.v2186-bite-callout',
  'top: var(--v2186-bite-top) !important;',
  'data-v2186-fishing-phase="bite"',
  '.v2186-gauge-cleanup-node',
  'body:not([data-screen="fishing"])',
  '.v2186-result-card',
  '.v2186-result-rarity-compact',
  '.v2186-result-impact',
  'grid-template-columns: repeat(2, minmax(0, 1fr)) !important;',
  '.v2186-village-stage-stable',
  '@media (max-height: 600px)',
]) if (!styles.includes(token)) fail(`src/styles.css missing ${token}`);
for (const token of [
  '## v2.1.86 변경사항',
  'v2186',
  '낚시 준비 화면의 바다물길/낚싯대/미끼 정보',
  '낚시 종료 후 남는 게이지/릴 콘솔/터치존 잔상',
  '오프닝 poster 미사용',
...
console.log('[v2186] fishing shelf, bite/result de-overlap, stale gauge cleanup, opening/player direction/package checks passed');
