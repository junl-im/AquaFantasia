...
const fail = (message) => { console.error(`[v2136] ${message}`); process.exit(1); };
...
if (pkg.version !== '2.1.36') fail(`package.json version is ${pkg.version}`);
if (lock.version !== '2.1.36' || lock.packages?.['']?.version !== '2.1.36') fail('package-lock.json version mismatch');
assertIncludes('src/data.ts', "APP_VERSION = '2.1.36'", 'APP_VERSION 2.1.36');
assertIncludes('src/data.ts', 'aqua-fantasia-v2.1.36-premium-polish-fishing-placement', 'v2.1.36 cache name');
assertIncludes('public/sw.js', 'aqua-fantasia-v2.1.36-premium-polish-fishing-placement', 'service worker cache');
assertIncludes('public/offline.html', 'v2.1.36', 'offline badge');
assertIncludes('README.md', '# AquaFantasia v2.1.36', 'README title v2.1.36');
if (fs.existsSync(path.join(root, 'APP_VERSION'))) fail('root APP_VERSION file must not exist in v2.1.36 full package');
...
for (const token of [
  'v2135-system-ui-fishing-engine',
  'v2135-system-ui-fishing-engine-root',
  'v2135-village-layout-grid-stability',
  'v2135-world-controls',
  'data-v2135-primary-order="plus-minus-build-center-shop-sail"',
  'v2135-fishing-system-screen',
  'dataset.v2135FishingSystem',
  'v2135-fishing-skill-strip',
  'data-v2135-command',
  'data-v2135-balance',
  'data-v2135-pressure',
  'v2135-reel-console',
  'v2135-reel-touch-zone',
  'v2135-shop-buy-price',
  ...
  if (!main.includes(token)) fail(`src/main.ts missing v2.1.36 token: ${token}`);
}
for (const token of [
  'V2135_OBJECT_FOOTPRINT_CLEARANCE_LOCK',
  'V2135_TILE_SNAP_ASSIST_LOCK',
  'footprintClearanceTileKeys',
  'nearestPlaceableOrigin',
  '가장 가까운 설치 가능 타일',
  'v2135-village-placement-engine-ready',
]) {
  if (!village.includes(token)) fail(`src/villageWorld.ts missing v2.1.36 token: ${token}`);
}
for (const token of [
  'v2135-system-ui-fishing-engine-root',
  '--v2135-aqua-card',
  ...
  '.v2135-shop-buy-price',
  '.v2135-fishing-skill-strip',
  '.v2135-console-readouts',
  '.v2135-reel-console',
  '.v2135-reel-touch-zone',
]) {
  if (!styles.includes(token)) fail(`src/styles.css missing v2.1.36 token: ${token}`);
}
for (const token of [
  '마을 배치 엔진',
  '낚시게임을 추가 개선',
  ...
  if (!read('README.md').includes(token)) fail(`README.md missing v2.1.35 note: ${token}`);
}



for (const token of [
  'v2136-premium-polish-fishing-placement',
  'v2136-premium-polish-fishing-placement-root',
  'v2136-village-premium-placement',
  'dataset.v2136PremiumPolish',
  'v2136-world-controls',
  'data-v2136-primary-order="plus-minus-build-center-shop-sail"',
  'v2136-fishing-polish-screen',
  'dataset.v2136FishingAssist',
  'v2136-fishing-coach',
  'data-v2136-assist',
  'v2136-reel-console',
  'v2136-reel-touch-zone',
  'v2136-shop-buy-price',
...
for (const token of [
  'V2136_PREMIUM_PLACEMENT_ASSIST_LOCK',
  'V2136_FINE_PLACEMENT_SEARCH_RADIUS = 3',
  'bestScore <= 1.20',
  'v2136-village-placement-assist-ready',
  '근처의 가장 가까운 설치 가능 타일',
  '시각 안전 간격',
]) {
  if (!village.includes(token)) fail(`src/villageWorld.ts missing v2.1.36 token: ${token}`);
...
for (const token of [
  ...
  '가까운 설치 가능 타일 보정 범위를 3칸까지 확장',
  '장력이 너무 낮을 때 감기, 너무 높을 때 풀기',
  '루트 APP_VERSION 파일 제거 정책 유지',
]) {
  if (!read('README.md').includes(token)) fail(`README.md missing v2.1.36 note: ${token}`);
}
...
console.log('[v2136] premium polish/fishing placement guard passed');
